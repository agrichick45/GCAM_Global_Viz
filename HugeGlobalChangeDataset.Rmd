...
```{r}
#This code calculates the cumulative regional emissions of each region
...
futReg_Region_NetEmissions.plt<-
  ggplot(data=dplyr::filter(Basin_Emission,
                                         Reg8 == "XLM" & year >2019 & EmissionType=="Complex")) +
  geom_line(mapping=aes(x=year, y=REF, color=Model), lwd=2) +
      #Add the custom color manual and labels
      scale_color_manual(values=c(ModelColors, name = "Emissions"), 
                      labels = c("FAO Glosis", "Houghton", "HWSD", "SG17", "SG20")) +
...
map_range$category <- factor(map_range$category, levels = c("Less than 1 Mt", "1 Mt - Less than 12 Mt", "12 Mt - Less than 25 Mt", "25 librMt - Less than 50 Mt", "Greater than 50 Mt"))
