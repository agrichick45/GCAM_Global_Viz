...
new_Shock_Emiss<- combined_shock_global_data  %>% 
  ...
  select(-c("series", "Emissions", "CO2", "LUC", "run", "Unit", "model_run_date", "script_run_date", "comments"))  %>%
  select(Model:'X2060') %>%
...
world_sum_cum_emiss<- wide_sum_cat_emiss %>%
  ...
  filter(Category != "UnmanagedLand") %>%
  filter(year >2015) %>%
  filter(Model == "FAO"|Model=="Houghton"|Model=="HWSD"|Model=="SG17") %>%
  group_by(Model, year) %>% 
  summarise(across(where(is.numeric), sum))
...
futFAOLUNetEmissions.plt<-ggplot()+
  geom_line(world_LU_FAO_cum_emiss, mapping=aes(x=year, y=DifCornShock, group=Category, color=Category), lwd=2) + 
  ggtitle("FAO Global Land Use Emission Change from Corn Shocks") + 
  labs(y="Difference from Baseline Mt")
```
...
futRegNetEmissions.plt<-ggplot() +
  ...
  geom_line(world_reg_sum_cum_emiss, mapping=aes(x=year, y=DifSoyShock, color=Model, linetype = "Soy"), lwd=2) +
      #Add the custom color manual and labels
      #scale_color_manual(values=c(ModelColors, name = "Emissions"), 
                      #labels = c("FAO Glosis", "Houghton")) +
  #Add the title==
